...
template <auto _minimum, auto _maximum, auto _step = 0> struct RosRange {
  static constexpr auto minimum = _minimum;
  static constexpr auto maximum = _maximum;
  static constexpr auto step    = _step;
  template <class T> static rfl::Result<T> validate(T _value) noexcept {
    ...
    if (_value < minimum || _value > maximum) {
      ...
      stream << "Value outside of expected range of [" << minimum << " , " << maximum << "], but got " << _value << ".";
      return rfl::error(stream.str());
    } else if constexpr (step != 0) {
      if constexpr (std::is_floating_point_v<T>) {
        if (_value != maximum && std::remainder((_value - minimum), step) != 0) {
          std::stringstream stream;
          stream << "value " << _value << " does not comply with step restriction min,max,step: " << minimum << " , "
                 << maximum << " ,  " << step << ".";
          return rfl::error(stream.str());
        }
      } else {
  ...
  template <class T> static rfl::parsing::schema::ValidationType to_schema() {
    using ValidationType = rfl::parsing::schema::ValidationType;
    const auto min_value = std::is_same_v<T, double> ? rfl::Variant<double, int>(static_cast<double>(_minimum))
                                                     : rfl::Variant<double, int>(static_cast<int>(_minimum));
    rfl::parsing::schema::ValidationType min_validation_type{
        .variant_ = rfl::parsing::schema::ValidationType::Minimum{min_value}};

    const auto max_value = std::is_same_v<T, int> ? rfl::Variant<double, int>(static_cast<double>(_maximum))
                                                  : rfl::Variant<double, int>(static_cast<int>(_maximum));
    rfl::parsing::schema::ValidationType max_validation_type{
        .variant_ = rfl::parsing::schema::ValidationType::Maximum{max_value}};
    if constexpr (step == 0) {

      return rfl::parsing::schema::ValidationType{
          .variant_ = rfl::parsing::schema::ValidationType::AllOf{.types_{min_validation_type, max_validation_type}}};
    } else {
...
template <class T> class is_ros_range;

template <class T> class is_ros_range : public std::false_type {};

template <auto _minimum, auto _maximum, auto step>
class is_ros_range<RosRange<_minimum, _maximum, step>> : public std::true_type {};

template <class T> constexpr bool is_ros_range_v = is_ros_range<std::remove_cvref_t<std::remove_pointer_t<T>>>::value;
...
template <typename T>
concept is_param = std::is_same_v<T, int> || std::is_same_v<T, double> || std::is_same_v<T, bool> ||
                   ...
                   std::is_same_v<T, std::vector<bool>> || std::is_same_v<T, std::vector<uint8_t>> ||
                   std::is_same_v<T, std::vector<std::string>> || std::is_same_v<T, std::vector<std::string>>;
...
template <typename T>
concept is_convertible_to_param = is_param<T> || (has_type_defined<T> && is_param<typename T::Type>) ||
...
template <typename T>
T declare_params(rclcpp::node_interfaces::NodeParametersInterface::SharedPtr parameter_interface,
  ...
  view.apply([&parameter_interface, &base_name]<typename Field>(const Field &field) {
    ...
    if constexpr (should_skip_deserialization<original_type>) {
      ...
      try {
        ...
        *field.value() = parameter_interface->declare_parameter(name, value.get_type(), descriptor).get<field_type>();
      ...
      if (result) {
        *field.value() = *result;
        ...
        throw rclcpp::exceptions::InvalidParameterValueException(" Parameter " + name + " is an enum but  " + name +
...
template <typename T>
T get_params(rclcpp::node_interfaces::NodeParametersInterface::SharedPtr parameter_interface,
             std::string base_name = "") {
  T retval;
  const auto view = rfl::to_view(retval);
  view.apply([&parameter_interface, &base_name]<typename Field>(const Field &field) {
      ...
      if (change.get_type() == value.get_type()) {
        try {
          *field.value() = change.get_parameter_value().get<field_type>();
          ...
          throw rclcpp::exceptions::InvalidParameterValueException("Parameter " + name +
                                                                   " value is invalid, error: " + e.what());
        }
      } else {
...
template <typename T>
rcl_interfaces::msg::SetParametersResult validate_param_changes(T prev_value,
                                                                const std::vector<rclcpp::Parameter> &parameter_changes,
  ...
  try {
    for (auto &change : parameter_changes) {
      view.apply([&change, base_name]<typename Field>(const Field &field) {
        process_parameter_change(field, change, base_name);
      });
    }
